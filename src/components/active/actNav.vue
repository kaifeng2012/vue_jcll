    ...
    <div class="activities content">
        ...
        <div class="lists">
          <div class="activeDetail" v-for="(item,index) in newsList">
            <router-link :to="{path: 'active/actDetails',query: {id:item.id}}" :key="" @:click="showCont">
              <div class="activePic">
              ...
              <div class="txt1">
                <span>{{item.activityName| ellipsis}}</span><i v-show="show" id="6b58fd0422b14bcba2932b9bd83b9541" name="redPoint"></i>
                <p>隶属街道：{{item.streetName}}</p>
                <p>邻里中心：{{item.centerName| ellipsis}}</p>
              </div>
            </router-link>
          </div>
        ...
        <div class="lists" style="display:none;">
          <div class="activeDetail" v-for="(item,index) in noticeList">
            <router-link :to="{path: 'active/actDetails',query: {id:item.id}}" :key="">
              <div class="activePic">
                <img :src="'http://www.shllzx.com/'+item.fileUrl" alt="">
               <!--  <div class="listIcon">
              ...
              <div class="txt1">
                <span>{{item.activityName| ellipsis}}</span>
                <p>隶属街道：{{item.streetName}}</p>
                <p>邻里中心：{{item.centerName| ellipsis}}</p>
              </div>
            </router-link>
...
export default {
    ...
    //alert(10);
        //新闻动态读取后台数据
        Axios.get('http://www.shllzx.com/llzx//activityController/getActivityList?newActivity=1&ext1=1').then(res=>{
          //console.log(res);
       ...
       // alert(index);
        ...
        if(this.num==0){
         // alert(this.num);
          ...
          //活动预告
           Axios.get('http://www.shllzx.com/llzx//activityController/getActivityList?activityStatus=1&ext1=1').then(res=>{
            //console.log(res);
          ...
          //历史活动
           Axios.get('http://www.shllzx.com/llzx//activityController/getActivityList?activityStatus=4&ext1=1').then(res=>{
            //console.log(res);
    ...
    watch: {
     ...
     filters: {
        ...
        ellipsis(value){//根据字符串的长度控制显示的字数超出显示省略号
          if(!value) return ''
            if(value.length>8){
